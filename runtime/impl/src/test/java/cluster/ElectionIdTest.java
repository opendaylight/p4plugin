...
public class ElectionIdTest {
    ...
    public void testGetDefaultId() {
        ElectionId electionId = new ElectionId((long)0, (long)0);
        Assert.assertEquals(electionId.getHigh(), ElectionIdGenerator.getInstance().getElectionId().getHigh());
        Assert.assertEquals(electionId.getLow(), ElectionIdGenerator.getInstance().getElectionId().getLow());
    }
