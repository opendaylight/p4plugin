...
public class Device {
    ...
    private org.opendaylight.p4plugin.p4runtime.proto.TableAction buildDirectTableAction(
        ...
        if (params != null) {
            params.forEach(p -> {
                ...
                BigInteger paramValue = p.getParamValue();
                ...
                if (paramValue != null) {
                    ...
                    paramBuilder.setValue(ByteString.copyFrom(paramValue.toByteArray(), 0, paramWidth));
                }
    ...
    private org.opendaylight.p4plugin.p4runtime.proto.FieldMatch buildExactMatchField(
        ...
        BigInteger exactValue = exact.getExactValue();

        if (exactValue != null) {
            exactBuilder.setValue(ByteString.copyFrom(exactValue.toByteArray(), 0, matchFieldWidth));
        }
    ...
    private org.opendaylight.p4plugin.p4runtime.proto.FieldMatch buildLpmMatchField(
        ...
        Integer matchFieldWidth = getMatchFieldWidth(tableName, fieldName);
        Integer matchFieldId = getMatchFieldId(tableName, fieldName);
        BigInteger lpmValue = lpm.getLpmValue();
        Long prefixLen = lpm.getPrefixLen();

        if (lpmValue != null && prefixLen != null) {
            lpmBuilder.setValue(ByteString.copyFrom(lpmValue.toByteArray(), 0, matchFieldWidth));
    ...
    private org.opendaylight.p4plugin.p4runtime.proto.FieldMatch buildTernaryMatchField(
        ...
        BigInteger ternaryValue = ternary.getTernaryValue();
        BigInteger mask = ternary.getMask();
        Integer matchFieldWidth = getMatchFieldWidth(tableName, fieldName);
        Integer matchFieldId = getMatchFieldId(tableName, fieldName);

        if (ternaryValue != null && mask != null) {
            ternaryBuilder.setValue(ByteString.copyFrom(ternaryValue.toByteArray(), 0, matchFieldWidth));
            ternaryBuilder.setMask(ByteString.copyFrom(mask.toByteArray(), 0, matchFieldWidth));
        }
